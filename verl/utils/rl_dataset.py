...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        # Filter out samples with no tumor
        if 'has_tumor' in self.dataset.column_names:
            original_count = len(self.dataset)
            self.dataset = self.dataset.filter(lambda x: x['has_tumor'] == True)
            filtered_count = len(self.dataset)
            removed_count = original_count - filtered_count
            print(f"Filtered out {removed_count} samples with no tumor (has_tumor=False)")
            print(f"Remaining samples: {filtered_count}")
        elif 'gt_slices' in self.dataset.column_names:
            # Fallback: some datasets may use gt_slices instead
            original_count = len(self.dataset)
            self.dataset = self.dataset.filter(lambda x: len(x['gt_slices']) > 0)
            filtered_count = len(self.dataset)
            removed_count = original_count - filtered_count
            print(f"Filtered out {removed_count} samples with no tumor (empty gt_slices)")
            print(f"Remaining samples: {filtered_count}")
