#!/bin/bash
# Multi-GPU Parallel Brain Tumor Evaluation Script
# Evaluates trained checkpoint on validation datasets using all available GPUs
...
# Model path - check if converted checkpoint exists, otherwise use base model
if [ -d "models/Qwen3-VL-8B-BrainTumor-Step200" ]; then
    MODEL_PATH="${MODEL_PATH:-models/Qwen3-VL-8B-BrainTumor-Step200}"
else
    # Fall back to base model (will need manual conversion of FSDP checkpoint)
    MODEL_PATH="${MODEL_PATH:-models/Qwen3-VL-8B-Thinking}"
    echo "WARNING: Using base model. To evaluate step 200, you need to convert the FSDP checkpoint first."
fi
...
# Output directory
OUTPUT_DIR="${OUTPUT_DIR:-eval_results/step200_$(date +%Y%m%d_%H%M%S)}"
...
print_header "Multi-GPU Brain Tumor Detection Evaluation"
...
if [ ! -d "$MODEL_PATH" ]; then
    ...
    ls -d brain_tumor_workdir/run_brain_tumor_image_4x80G/global_step_*/actor/huggingface 2>/dev/null || echo "  No checkpoints found"
    echo ""
    echo "To evaluate a different checkpoint:"
    echo "  export MODEL_PATH=brain_tumor_workdir/run_brain_tumor_image_4x80G/global_step_150/actor/huggingface"
    echo "  bash evaluation_scripts/run_eval_brain_tumor_parallel.sh"
    exit 1
fi
...
print_info ""
print_info "To evaluate a different checkpoint:"
echo "  export MODEL_PATH=brain_tumor_workdir/run_brain_tumor_image_4x80G/global_step_150/actor/huggingface"
echo "  bash evaluation_scripts/run_eval_brain_tumor_parallel.sh"
