...
if [ ! -d "$MODEL_PATH" ]; then
    ...
    ls -d models/* 2>/dev/null || echo "  No models found in models/"
    echo ""
    ...
    echo "  export MODEL_PATH=brain_tumor_workdir/run_brain_tumor_image_4x80G/global_step_400/actor"
    echo "  bash evaluation_scripts_multiimage/run_eval_multiimage.sh"
    exit 1
fi
...
for i in "${!DATASETS[@]}"; do
    ...
    # Build command with multi-image support
    CMD="CUDA_VISIBLE_DEVICES=$GPU_ID python evaluation_scripts_multiimage/eval_brain_tumor.py \
...
print_info ""
print_info "To evaluate a different model/checkpoint:"
echo "  export MODEL_PATH=brain_tumor_workdir/run_brain_tumor_image_4x80G/global_step_400/actor"
echo "  bash evaluation_scripts_multiimage/run_eval_multiimage.sh"
