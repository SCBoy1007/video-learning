...
def create_prompt(problem: str, image_size: int, system_prompt: str) -> str:
    """
...
def main():
    ...
    for idx, item in enumerate(tqdm(dataset, desc="Preparing")):
        ...
        if "image" in item:
            ...
            print(f"Warning: No image found in sample {idx}")
            continue
        ...
        if image.size != (args.image_size, args.image_size):
            image = image.resize((args.image_size, args.image_size), PILImage.Resampling.BILINEAR)

        # Get problem text
        problem = item.get("problem", "locate brain tumor in this mri image")

        # Create message
        msg = create_prompt(problem, args.image_size, args.system_prompt)
        # Replace text content with image + text structure
        msg[1]["content"] = [
            {"type": "image", "image": image},
            {"type": "text", "text": msg[1]["content"]}
        ]
