...
# ==================== TUMOR EXISTENCE DETECTION EVALUATION WITH VISUALIZATION ====================
# This script evaluates tumor existence detection (yes/no) and saves visualization images
# Output: Accuracy metrics + individual prediction visualizations
# ===================================================================================================

def parse_args():
    parser = argparse.ArgumentParser(description="Tumor existence detection evaluation with visualization")
    ...
    parser.add_argument("--vis_dir", type=str, default="visual_results",
...
def visualize_prediction(image, prediction, ground_truth, thinking, ann_id, vis_dir):
    """
    ...
    Args:
        ...
        ann_id: annotation ID for filename
    ...
    text_color = 'red' if prediction == 'yes' else 'green'

    # Add colored border
    ax = plt.gca()
    rect = patches.Rectangle((0, 0), image.width, image.height,
    ...
    plt.text(
        ...
        color=text_color,
        weight='bold',
        bbox=dict(boxstyle='round', facecolor='white', alpha=0.8, edgecolor=border_color, linewidth=3)
    )

    result_str = "✓ CORRECT" if correct else "✗ WRONG"
    plt.title(f'{result_str}\nGT: {ground_truth.upper()}', fontsize=12, color=border_color, weight='bold')
    plt.axis('off')

    # Thinking process
    plt.subplot(1, 3, 3)
    plt.text(
        0.05, 0.95, f"Thinking Process:\n\n{thinking[:300]}...",
    ...
    vis_path = os.path.join(vis_dir, f"{ann_id}_{prediction}_gt{ground_truth}.png")
    plt.savefig(vis_path, dpi=100, bbox_inches='tight')
    plt.close()

    # Save thinking to text file
    txt_path = os.path.join(vis_dir, f"{ann_id}_{prediction}_gt{ground_truth}.txt")
...
def main():
    ...
    print("="*80)
    print(f"Tumor Existence Detection Evaluation (with Visualization) - Part {args.idx+1}/{args.num_parts}")
    print("="*80)
    ...
    if 'has_tumor' not in dataset[0]:
        print("WARNING: 'has_tumor' field not found. Cannot compute accuracy.")
    ...
    for item in dataset:
        ...
        metadata = {
            "image_id": item.get("image_id", f"img_{len(metadata_list)}"),
            "ann_id": item.get("ann_id", f"ann_{len(metadata_list)}"),
            "image": image,
        }
        if has_ground_truth:
    ...
    for i in tqdm(range(0, len(messages), args.batch_size), desc="Inference"):
        ...
        for output_idx, output_text in enumerate(batch_output_text):
            try:
                ...
                if has_ground_truth:
                    ground_truth = "yes" if batch_metadata[output_idx]["has_tumor"] else "no"
                    result["ground_truth"] = ground_truth
                    result["correct"] = (answer == ground_truth)

                    # Visualize
                    visualize_prediction(
                        batch_metadata[output_idx]["image"],
                        answer,
                        ground_truth,
                        thinking,
                        batch_metadata[output_idx]["ann_id"],
                        args.vis_dir
                    )
                ...
                print(f"\nError processing {batch_metadata[output_idx]['ann_id']}: {e}")
                result = {
                    "image_id": batch_metadata[output_idx]["image_id"],
                    "ann_id": batch_metadata[output_idx]["ann_id"],
                    "prediction": "error",
